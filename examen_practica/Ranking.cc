...
void Ranking::imprimeix_millor_jugador_sets() {
  string j = ranking[0]->first;
  double x = ranking[0]->second.percentatge_sets_guanyats();
  for (int i = 1; i < ranking.size(); ++i) {
    double y = ranking[i]->second.percentatge_sets_guanyats();
    if (y > x) {
      j = ranking[i]->first;
      x = y;
    }
